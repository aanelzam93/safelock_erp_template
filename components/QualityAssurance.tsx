...
const statusStyles: { [key in QualityCheck['status']]: string } = {
...
const QualityAssurance: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        return QUALITY_CHECK_DATA.filter(item => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Quality Assurance</h1>
                    <p className="text-sm text-gray-500">
                        <span className="text-[#4fd1c5] cursor-pointer hover:underline">Beranda</span> &gt; QA
                    </p>
                </div>
                 <button className="bg-[#4fd1c5] text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-500 transition flex items-center">
                    <PlusIcon className="w-5 h-5 mr-1"/> Log New QC
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                            <tr>
                                {['Check ID', 'Product Name', 'Batch ID', 'Check Date', 'Inspector', 'Status', 'Actions'].map(header => (
                                    <th key={header} scope="col" className="px-6 py-3 font-semibold">{header}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {paginatedData.map(qc => (
                                <tr key={qc.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-mono text-teal-800">#{qc.id}</td>
                                    <td className="px-6 py-4 font-medium text-gray-900">{qc.productName}</td>
                                    <td className="px-6 py-4">{qc.batchId}</td>
                                    <td className="px-6 py-4">{qc.checkDate}</td>
                                    <td className="px-6 py-4">{qc.inspector}</td>
                                    <td className="px-6 py-4">
                                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${statusStyles[qc.status]}`}>
                                            {qc.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
