...
import { COST_ACCOUNTING_DATA } from '../constants';
...
const CostAccounting: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        return COST_ACCOUNTING_DATA.filter(item => 
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Cost Accounting (HPP)</h1>
                    <p className="text-sm text-gray-500">
                        <span className="text-[#4fd1c5] cursor-pointer hover:underline">Finance</span> &gt; Cost Accounting
                    </p>
                </div>
                <button className="bg-[#4fd1c5] text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-500 transition flex items-center">
                    <PlusIcon className="w-5 h-5 mr-1"/> Recalculate HPP
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                            <tr>
                                {['Cost Center Name', 'HPP (Harga Pokok Produksi)', 'Last Calculated', 'Actions'].map(header => (
                        ...
                        <tbody>
                            {paginatedData.map(item => (
                                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 font-semibold">Rp {item.hpp.toLocaleString('id-ID')}</td>
                                    <td className="px-6 py-4">{item.lastCalculated}</td>
