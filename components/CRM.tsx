...
const statusStyles: { [key in Customer['status']]: string } = {
...
const CRM: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        return CUSTOMER_DATA.filter(item => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Customer Relationship Management (CRM)</h1>
                    <p className="text-sm text-gray-500">
                        <span className="text-[#4fd1c5] cursor-pointer hover:underline">Beranda</span> &gt; CRM
                    </p>
                </div>
                 <button className="bg-[#4fd1c5] text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-500 transition flex items-center">
                    <PlusIcon className="w-5 h-5 mr-1"/> Tambah Pelanggan
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                            <tr>
                                {['Customer Name', 'Contact Person', 'Email / Phone', 'Status', 'Actions'].map(header => (
                        ...
                        <tbody>
                            {paginatedData.map(customer => (
                                <tr key={customer.id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${statusStyles[customer.status]}`}>
                                            {customer.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
