...
import { RECEIVABLE_DATA } from '../constants';
...
const statusStyles: { [key in Receivable['status']]: string } = {
...
const AccountsReceivable: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        return RECEIVABLE_DATA.filter(item => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Accounts Receivable (Piutang)</h1>
                    <p className="text-sm text-gray-500">
                        <span className="text-[#4fd1c5] cursor-pointer hover:underline">Finance</span> &gt; Accounts Receivable
                    </p>
                </div>
                 <button className="bg-[#4fd1c5] text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-500 transition flex items-center">
                    <PlusIcon className="w-5 h-5 mr-1"/> Buat Invoice
                </button>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                            <tr>
                                {['Invoice ID', 'Customer', 'Issue Date', 'Due Date', 'Amount', 'Status', 'Actions'].map(header => (
